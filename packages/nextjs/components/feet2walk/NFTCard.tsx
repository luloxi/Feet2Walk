...
const NFTCard: React.FC<NFTCardProps> = ({ data, tokenId, owner, connectedAddress }) => {
  ...
  const { writeAsync: endWalk } = useScaffoldContractWrite({
    contractName: "FeetCoordinator",
    functionName: "endWalk",
    args: [BigInt(tokenId)],
  });

  const handleEndWalk = async (tokenId: number) => {
    await endWalk({ args: [BigInt(tokenId)] });
  };
  ...
  const { writeAsync: approve } = useScaffoldContractWrite({
    ...
    args: [feetCoordinatorData ? feetCoordinatorData.address : undefined, BigInt(0)],
  });

  const handleApprove = async (to: string, tokenId: number) => {
    if (!feetCoordinatorData || !feetCoordinatorData.address) {
      console.error("Feet coordinator data is not available.");
      return;
    }
    await approve({ args: [feetCoordinatorData.address, BigInt(tokenId)] });
  };
    ...
    >
      ...
      {activeTab === "artwork" && (
        ...
        <figure className="relative py-4">
          <Image src={data.image} alt="NFT Image" className="h-64 min-w-full" width={150} height={150} />
        ...
        <>
          ...
          <div className="card-actions mb-2 justify-around">
            <button
              ...
              onClick={() => handleApprove(transferAddresses[tokenId], tokenId)}
            >
              Approve
            </button>
            <button
              className="btn btn-primary bg-green-500 btn-md px-8 m-2 tracking-wide"
              onClick={() => handleEndWalk(tokenId)}
            >
              End Walk
            </button>
          </div>
